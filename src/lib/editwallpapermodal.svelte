<script lang="ts">
    ...
    function closeModalOnOutsideMouseUp(event) {
      if (!mouseDownInsideModal && event.target.classList.contains("modal-overlay")) {
        toggleModal();
      }
    ...
    <div class="modal-overlay" on:click={closeModalOnOutsideMouseUp}>
      <div class="modal" on:mousedown={handleModalMouseDown}>
        ...
        <div class="image-upload-form">
          <input id="fileInput" type="file" accept=".jpg, .jpeg, .png" on:change={onImageChange} style="display: none;" />
        ...
        <p style="text-align:center; background-color: #41414136; padding: 0.5rem; border-radius: 0.2rem 0.2rem 0rem 0rem;">Note: Only .jpg and .jpeg files are accepted.</p>
        <p style="text-align:center; background-color: #41414136; padding: 0.5rem; border-radius: 0rem 0rem 0.2rem 0.2rem;">Note 2: There's also a 10mb file upload limit</p>
  
        <br>
        {#if selectedFile}
        <p>{imgName}</p>
        <br>
        <button style="width: 50%;" on:click={uploadImageToServer}>Upload</button>
      {/if}

      {#if !selectedFile}
      <button class="remove-wp-btn" on:click={removeWallpaper}>Remove Current Wallpaper</button>
      {/if}
